...
def _simplex_initial_logits(dim: int, n_init: int, seed: int) -> np.ndarray:
    ...
    We include the uniform mixture plus Dirichlet draws so reduced methods do
    not start from artificially spiky softmax samples in low dimensions.
    """
...
def _fit_gpytorch_mll_with_warning(mll, context: str) -> bool:
    """Fit a GP marginal likelihood, surfacing failures to the caller."""
    try:
        fit_gpytorch_mll(mll)
        return True
    except Exception as exc:
        warnings.warn(
            f"{context}: GP fit failed ({type(exc).__name__}: {exc}). "
...
def vanilla_bo(objective, d: int, budget: int = 30, n_init: int = 5,
    ...
    for _ in range(budget - n_init):
        ...
        if not _fit_gpytorch_mll_with_warning(mll, f"vanilla_bo(seed={seed})"):
            z_new = rng.uniform(z_lo, z_hi, size=d)
...
def reduced_bo(objective, d_full: int, selected_indices: List[int],
    ...
    for _ in range(budget - n_init):
        ...
        gp = SingleTaskGP(train_Z_norm, train_Y, outcome_transform=Standardize(m=1))
        mll = ExactMarginalLogLikelihood(gp.likelihood, gp)
        if not _fit_gpytorch_mll_with_warning(
            mll, f"reduced_bo(seed={seed}, k={k})"
        ):
            z_new = rng.uniform(z_lo, z_hi, size=k)
...
def random_subset_bo(objective, d_full: int, k: int = 5,
    ...
    return reduced_bo(objective, d_full, selected_indices,
...
def rembo_bo(objective, d: int, budget: int = 30, n_init: int = 5,
    ...
    z_radius = float(np.sqrt(k))
    z_bounds = torch.tensor(
        [[-z_radius] * k, [z_radius] * k],
        dtype=torch.double,
    )
    ...
    z_init = unnormalize(sobol_z.draw(n_init).double(), z_bounds)
    for i in range(n_init):
    ...
    for _ in range(budget - n_init):
        ...
        gp = SingleTaskGP(train_Z_norm, train_Y, outcome_transform=Standardize(m=1))
        mll = ExactMarginalLogLikelihood(gp.likelihood, gp)
        if not _fit_gpytorch_mll_with_warning(mll, f"rembo_bo(seed={seed}, k={k})"):
            z_new = rng.uniform(-z_radius, z_radius, size=k)
            y_new = objective(z_to_x(torch.tensor(z_new, dtype=torch.double)))
            all_z.append(np.array(z_new, dtype=float))
            all_y.append(y_new)
            best_values.append(max(all_y))
            continue
...
def turbo_bo(objective, d: int, budget: int = 30, n_init: int = 5,
    ...
    for _ in range(budget - n_init):
        ...
        gp = SingleTaskGP(train_Z_norm, train_Y, outcome_transform=Standardize(m=1))
        mll = ExactMarginalLogLikelihood(gp.likelihood, gp)
        if not _fit_gpytorch_mll_with_warning(mll, f"turbo_bo(seed={seed})"):
            z_new = rng.uniform(z_lo, z_hi, size=d)
            y_new = objective(_softmax(z_new))
            all_z.append(z_new)
            all_y.append(y_new)
        ...
        lengthscales = lengthscales.clamp_min(1e-12)
        weights = lengthscales.mean() / lengthscales
...
def saasbo_bo(objective, d: int, budget: int = 30, n_init: int = 5,
    ...
    for _ in range(budget - n_init):
        ...
        try:
            ...
            warnings.warn(
                f"saasbo_bo(seed={seed}): NUTS fit failed "
                f"({type(exc).__name__}: {exc}). Falling back to SingleTaskGP.",
                RuntimeWarning,
                stacklevel=2,
            )
            gp = SingleTaskGP(train_Z_norm, train_Y, outcome_transform=Standardize(m=1))
            mll = ExactMarginalLogLikelihood(gp.likelihood, gp)
            fit_ok = _fit_gpytorch_mll_with_warning(mll, f"saasbo_bo(seed={seed}) fallback")
...
def sebo_bo(objective, d: int, budget: int = 30, n_init: int = 5,
    ...
    for step in range(n_bo_steps):
        ...
        try:
            ...
            warnings.warn(
                f"sebo_bo(seed={seed}): NUTS fit failed "
                f"({type(exc).__name__}: {exc}). Falling back to SingleTaskGP.",
                RuntimeWarning,
                stacklevel=2,
            )
            gp = SingleTaskGP(train_Z_norm, train_Y, outcome_transform=Standardize(m=1))
            mll = ExactMarginalLogLikelihood(gp.likelihood, gp)
            fit_ok = _fit_gpytorch_mll_with_warning(mll, f"sebo_bo(seed={seed}) fallback")
...
def run_single_category(
    ...
    method_registry = {
        "llm":            (f"LLM + BO (k={k})",           lambda s: reduced_bo(objective, d, selected, budget=budget, n_init=n_init, seed=s)),
    ...
    return {
        ...
        "target": {
            NUTRIENT_LABELS[col]: float(MEAT_TARGETS[category][col])
            for col in NUTRIENT_COLUMNS
        },
        "approx_profile": {
...
def plot_category_convergence(
    ...
    path = os.path.join(save_dir, f"meat_{category.replace(' ', '_').replace('&', 'and')}_convergence{suffix}.png")
...
def plot_category_bar(
    ...
    plt.tight_layout()
    suffix = f"_{run_tag}" if run_tag else ""
    path = os.path.join(save_dir, f"meat_{category.replace(' ', '_').replace('&', 'and')}_final{suffix}.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"  saved {path}")
...
def plot_summary_heatmap(
    ...
    plt.tight_layout()
    suffix = f"_{run_tag}" if run_tag else ""
    path = os.path.join(save_dir, f"meat_summary_heatmap{suffix}.png")
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"  saved {path}")
...
def run_all(
    ...
    summary_path = os.path.join(results_dir, f"meat_experiments_{timestamp}.json")
    raw_path = os.path.join(results_dir, f"meat_experiments_raw_{timestamp}.json")
