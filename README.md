...
Not every ingredient has to be searched for. Give a row a **Formula** — `= batch size − Water − Salt`, or `= rest` for the one row that takes whatever is left of the batch size — and the app works that amount out from the other rows every time. A limit over several ingredients can say **Exactly** as well as At least and At most, in grams or as a **% of batch size**.
